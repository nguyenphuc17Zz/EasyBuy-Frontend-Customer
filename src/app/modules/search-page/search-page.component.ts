...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, HeaderComponent, SidebarComponent, FooterComponent, FormsModule],
...
export class SearchPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => { // mỗi lần param thay đổi nó chạy code này 
  ...
  async getAllProducts() {
    ...
    try {
      ...
      this.products = pros;
      this.products = this.products.filter(p => p.productName.toLowerCase().includes(this.searchKey) || p.model.toLowerCase().includes(this.searchKey) || p.gender.toLocaleLowerCase().includes(this.searchKey))
      this.executeFilter();
      // TINH TONG TRANG 
